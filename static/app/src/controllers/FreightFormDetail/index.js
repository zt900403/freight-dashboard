            ...
            <Collapse bordered={false} defaultActiveKey={['1', '2', '3', '4', '5', '6']}>
                <Panel header="基本录入" key="1">
                    ...
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        ...
                        <Col span={6}>
                            {flagStep2 ? <p><strong>A销售量小计: </strong><span
                                className="red">{this.add(data.ASellAmountAdjust, data.ASellAmount)}</span></p> : ''}
                        </Col>
                        <Col span={6}>
                    ...
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        <Col span={6}>
                            {admin ?
                                <p><strong>A销售合计: </strong><span className="red">{this.multiply(data.ASellUnitPrice,
                    ...
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        ...
                        <Col span={6}>
                            {admin ? <p><strong>A采购小计价: </strong><span
                                className="red">{this.multiply(data.ABuyUnitPrice, data.APurchaseAmount)}</span>
                            </p> : ''}
                        </Col>
                        <Col span={6}>
                    ...
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        <Col span={6}>
                            {flagStep3 ? <p><strong>A补贴运费小计: </strong><span
                                className="red">{this.multiply(data.AFreightSubsidy, data.APurchaseAmount)}</span>
                            </p> : ''}
                        </Col>
                        <Col span={6}>
                        ...
                        <Col span={6}>
                            {admin ? <p><strong>运费小计: </strong><span
                                className="red">{this.add(this.multiply(data.freightUnitPrice, data.freightPriceTonsAdjust),
                                data.otherAddItem)}</span></p> : ''}
                        </Col>
                        {   data.needZhebaiCalc ?
                            <Col span={6}>
                                {flagStep2 ? <p><strong>浓度:</strong> {data.concentration}%</p> : ''}
                            </Col>
                            :
                            ''
                        }
                    </Row>
                    {
                        data.needZhebaiCalc ? (
                            <Row>
                                <Col span={6}>
                                    {flagStep2 ? <p><strong>A折百量:</strong><span className="red">{(this.multiply(data.ASellAmount, data.concentration)*0.01).toFixed(2)}</span></p> : ''}
                                </Col>
                                <Col span={6}>
                                    {flagStep2 ? <p><strong>B折百量:</strong><span className="red">{(this.multiply(data.BSellAmount, data.concentration)*0.01).toFixed(2)}</span></p> : ''}
                                </Col>
                                <Col span={6}>
                                    {flagStep2 ? <p><strong>C折百量:</strong><span className="red">{(this.multiply(data.CSellAmount, data.concentration)*0.01).toFixed(2)}</span></p> : ''}
                                </Col>
                                <Col span={6}>
                                    {flagStep2 ? <p><strong>D折百量:</strong><span className="red">{(this.multiply(data.DSellAmount, data.concentration)*0.01).toFixed(2)}</span></p> : ''}
                                </Col>
                            </Row>)
                            :
                            ''
                ...
                <Panel header="留货车辆录入" key="2">
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        ...
                        <Col span={6}>
                            {flagStep1 ? <p><strong>留货车辆费用小计: </strong><span
                                className="red">{this.multiply(data.keepCarUnitPrice, data.keepProductAmount)}</span>
                            </p> : ''}
                        </Col>
                        <Col span={6}>
                ...
                <Panel header="多点采购信息录入" key="3">
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        ...
                        <Col span={6}>
                            {admin ? <p><strong>B采购小计价: </strong><span
                                className="red">{this.multiply(data.warehouseSupplement, data.BBuyUnitPrice)}</span>
                            </p> : ''}
                        </Col>
                        <Col span={6}>
                    ...
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        <Col span={6}>
                            {admin ? <p><strong>B补贴运费小计: </strong><span
                                className="red">{this.multiply(data.BFreightSubsidy, data.warehouseSupplement)}</span>
                            </p> : ''}
                        </Col>
                    </Row>
                </Panel>
                <Panel header="第三方贸易商信息录入" key="4">
                    ...
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        <Col span={6}>
                            {admin ? <p><strong>B销售量小计: </strong><span
                                className="red">{this.add(data.BSellAmountAdjust, data.BSellAmount)}</span></p> : ''}
                        </Col>
                        <Col span={6}>
                        ...
                        <Col span={6}>
                            {admin ?
                                <p><strong>B销售合计: </strong><span className="red">{this.multiply(data.BSellUnitPrice,
                    ...
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        <Col span={6}>
                            {admin ? <p><strong>C销售量小计: </strong><span
                                className="red">{this.add(data.CSellAmountAdjust, data.CSellAmount)}</span></p> : ''}
                        </Col>
                        <Col span={6}>
                        ...
                        <Col span={6}>
                            {admin ?
                                <p><strong>C销售合计: </strong><span className="red">{this.multiply(data.CSellUnitPrice,
                    ...
                    <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                        <Col span={6}>
                            {admin ? <p><strong>D销售量小计: </strong><span
                                className="red">{this.add(data.DSellAmountAdjust, data.DSellAmount)}</span></p> : ''}
                        </Col>
                        <Col span={6}>
                        ...
                        <Col span={6}>
                            {admin ?
                                <p><strong>D销售合计: </strong><span className="red">{this.multiply(data.DSellUnitPrice,
